...
import { cases, Case, Category } from '../data/cases';
                ...
                <div className="flex flex-col gap-8 border-b border-zinc-900 pb-12">
                    <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                        <h2 className="text-4xl md:text-6xl text-white font-semibold leading-[1.2]">
                            <Typewriter text="КАТАЛОГ РЕШЕНИЙ" speed={0.05} />
                        </h2>
                ...
                {selectedCase && (
                    <>
                        ...
                        >
                            ...
                            {/* Details Side */}
                            <div className="w-full md:w-[40%] lg:w-[35%] p-8 md:p-10 lg:p-12 flex flex-col gap-6 overflow-y-auto relative">
                                <button
                                    onClick={() => setSelectedCase(null)}
                                    className="absolute top-6 right-6 hidden md:block text-zinc-500 hover:text-white z-20"
                                >
                                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>
                                </button>

                                <h3 className="text-3xl text-white font-semibold leading-tight">{selectedCase.title}</h3>

                                {selectedCase.metrics && (
                                    <span className="text-xs bg-white/5 text-zinc-300 px-3 py-1 rounded-full font-mono w-fit">
                                        {selectedCase.metrics}
                                    </span>
                                )}
                                ...
                                <div className="whitespace-pre-line text-zinc-400 leading-relaxed text-sm md:text-base">
                                    {selectedCase.fullDesc || selectedCase.desc}
                                </div>

                                <div className="flex flex-wrap gap-2 mt-auto pt-4">
                                    {selectedCase.tags.map((tag) => (
                                        <span key={tag} className="text-[10px] text-zinc-600 border border-zinc-900 px-2 py-1 rounded-sm font-mono uppercase tracking-wider">
                                            #{tag}
                                        </span>
                                    ))}
                                </div>
