...
export function AiAssistant() {
    ...
    const initSpeechRecognition = useCallback(() => {
        ...
        recognition.onerror = () => {
            ...
            setTimeout(() => {
                if (isOpen && !isSpeaking && !isProcessing) startListening();
            }, 1000);
        };
    ...
    useEffect(() => {
        if (isOpen) {
            ...
            if (recognitionRef.current && !isSpeaking) startListening();
        } else {
            stopListening();
            setCurrentTranscript('');
        }
    ...
    return (
        <div className="fixed bottom-8 right-8 z-[200] flex flex-col items-end gap-4">
                        ...
                        >
                            ...
                            <div className="relative px-6 py-8 flex flex-col items-center">
                                        ...
                                        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                                            {[...Array(3)].map((_, i) => (
                                                <motion.div
                                                    ...
                                                    animate={{ width: [80, 130], height: [80, 130], opacity: [0.4, 0] }}
                                ...
                                <p className="mt-4 text-[11px] text-white/40 text-center max-w-[90%] leading-relaxed">
                                    {history.length > 0
                                        ? history[history.length - 1].content.slice(0, 120) + (history[history.length - 1].content.length > 120 ? '...' : '')
                                        : isListening
                                            ? 'Говорите — спросите про курс'
                                            : 'Спросите меня про обучение'}
                                </p>
                            </div>
            ...
            {/* Floating Button — прежняя иконка */}
            <div className="relative">
                ...
                <motion.button
                    ...
                    style={{
                        ...
                        boxShadow: isOpen ? 'none' : '0 0 40px rgba(0,212,255,0.4), inset 0 1px 0 rgba(255,255,255,0.1)'
                    }}
                >
                    <div
                        className="absolute inset-0 rounded-full p-[2px]"
                        style={{ background: 'linear-gradient(135deg, #00d4ff 0%, #0066ff 50%, #00d4ff 100%)' }}
                    >
                        ...
                        <>
                            ...
                            >
                                <svg viewBox="0 0 60 60" className="w-full h-full">
                                    <circle cx="30" cy="30" r="26" fill="none" stroke="#00d4ff" strokeWidth="1.5" strokeDasharray="15 40" opacity="0.8" />
                                </svg>
                            </motion.div>
                            <motion.div
                            ...
                            >
                                <svg viewBox="0 0 60 60" className="w-full h-full">
                                    <circle cx="30" cy="30" r="22" fill="none" stroke="#ff00ff" strokeWidth="1" strokeDasharray="8 50" opacity="0.5" />
                                </svg>
                            </motion.div>
                            <motion.div
                        ...
                        {isOpen ? (
                            ...
                            <div className="relative">
                                <motion.div
                                    ...
                                    transition={{ duration: 0.2, repeat: Infinity, repeatDelay: 2, times: [0, 0.25, 0.5, 0.75, 1] }}
                                >
                                ...
                                <motion.div
                                    className="absolute inset-0"
                                    animate={{ x: [0, 2, -2, 1, 0], opacity: [1, 0.8, 1, 0.9, 1] }}
                                    transition={{ duration: 0.2, repeat: Infinity, repeatDelay: 2, delay: 0.05, times: [0, 0.25, 0.5, 0.75, 1] }}
                                >
                                    <Mic size={28} className="text-blue-500/50" />
                                </motion.div>
                                <motion.div
                                    animate={{ opacity: [1, 0.7, 1, 0.9, 1, 0.8, 1], scale: [1, 1.02, 1, 0.98, 1] }}
